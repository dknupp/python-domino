...
class Domino:
    def __init__(self, project, api_key=None, host=None, domino_token_file=None, auth_token=None):
        ...
        self.request_manager = self._authenticate_request_manager(api_key, auth_token, domino_token_file)
    ...
    def _authenticate_request_manager(self, api_key, auth_token, domino_token_file):
        """
        """
        ...
        assert any(auth_type for auth_type in auth_types.values()), \
                  (f"Unable to authenticate: no authentication value provided in "
                   "{pprint.pformat(auth_types)}")

        return _HttpRequestManager(DominoAuth(**auth_types))
